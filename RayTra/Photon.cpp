...
Photon::Photon(Vector3d pos, Vector3d dir, Vector3d intensities, w_ptr<Light> light) : RayBase(pos, dir, RayBase::PHOTON), m_intensities(intensities), m_light(light)
{
	...
	m_atten.push_back(Vector3d::Zero());
	onlySpecular = false;
}

Photon::Photon(Vector3d pos, Vector3d dir, Vector3d intensities, w_ptr<Light> light, std::vector<double> ref, std::vector<Vector3d> alpha) : RayBase(pos, dir, RayBase::PHOTON), m_intensities(intensities), m_refIndices(ref), m_atten(alpha), m_light(light)
{
	Photon::count++;
	onlySpecular = false;
}
